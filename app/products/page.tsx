...
export default async function ProductsPage({
  ...
  searchParams: Promise<{ category?: string; sort?: string; seller?: string }>;
}) {
  const {
    category = 'all',
    sort = 'newest',
  ...
  if (seller) {
    products = products.filter((p: any) => p.seller === seller || p.seller?._id === seller);
  }
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              {/* Persistant filter clearing regardless if there's products */}
              {(category !== 'all' || seller) && (
                <div className="flex items-center gap-2 mt-1">
                  {category !== 'all' && (
                    <Link
                      href={`/products${seller ? `?seller=${seller}&sort=${sort}` : `?sort=${sort}`}`}
                      className="text-xs text-terracotta font-semibold hover:underline"
                    >
                      Clear category filter
                    </Link>
                  )}
                  {category !== 'all' && seller && <span className="text-xs text-subheading">·</span>}
                  {seller && (
                    <Link
                      href={`/products${category !== 'all' ? `?category=${category}&sort=${sort}` : `?sort=${sort}`}`}
                      className="text-xs text-terracotta font-semibold hover:underline"
                    >
                      Clear shop filter
                    </Link>
