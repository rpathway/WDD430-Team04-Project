...
export default function ProductDetailPage() {
  ...
  async function handleAddToCart() {
    try {
      ...
      await refreshCart(); // <-- updates header count
    } catch (error: any) {
      ...
      <div className="md:grid md:grid-cols-2 md:gap-10 lg:gap-16">
        ...
        <div className="mt-6 md:mt-0">
          ...
          <button
            ...
            className="flex-1 bg-terracotta text-white text-sm font-bold py-3.5 rounded-xl hover:bg-terra-dark transition-colors shadow-sm disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
          >
            {addingToCart
              ? "Adding..."
              : inStock
            ...
            <button onClick={() => setWishlisted(!wishlisted)}
              className={`flex-1 text-sm font-bold py-3.5 rounded-xl border transition-colors ${wishlisted ? 'bg-terra-dark/20 text-terracotta border-terracotta' : 'bg-white text-charcol border-light-orange hover:border-terracotta hover:bg-terra-dark/20'} cursor-pointer`}>
